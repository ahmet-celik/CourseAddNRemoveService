...
public class CourseAddRemoveService {
	private static  final IMap<String, Student> 		studentsMap= Hazelcast.getMap("students-map"); // student->courses
	private static  final IMap<String, Course> 		coursesMap =  Hazelcast.getMap("courses-map"); // course->students
	...
	public CourseAddRemoveService(List<Student> students,List<Course> courses,List<Instructor> inst){
		 ...
		 try {
			if(studentsMap.size()==0 && coursesMap.size()==0){
					...
					for(Instructor i:inst){
						for(String c:i.getCoursesTeaching()){
							coursesMap.addEntryListener(i, c,true);
						}
					}
					System.out.println("# of students: "+studentsMap.size()+"\n# of courses: "+coursesMap.size());
			}
...
//		studentsMap.lock(studentID);
		...
		try{
		    ...
		    if(!course.valid()) throw new IllegalStateException("Max Quota Limit Passed!!");
